...
const clearedTrial = null; // Consumed at staging; exact unused capsule exists locally, no activation yet.
const reviewed = {
    trial: '20260923T103000Z-1',
    ...
    files: {
        'NativeHost.qml': '26fd85c01ee37869a38dd1fa22d0cf27ed352e79de2ab733606e6ce930d54d56',
        ...
        'companion-notebook.qmd': '71824e22e9a2a5d05a2991763bc4431017fb0c2789075637c1cf24450a3442f6',
        'probe.sh': '1a4b2c6adc9d6e39df7e03d7440efabea721b3456aadf48f4c1d2b93f6fb4aef',
...
assert.equal(clearedTrial, reviewed.trial, 'Lifecycle disposable trial requires a fresh exact-capsule review');
const id = process.argv[2];
assert.equal(id, reviewed.trial, 'Only the specifically reviewed lifecycle trial may stage');
const payload = 'build/lifecycle-native', stage = 'build/probe-' + id;
try { fs.lstatSync(stage); assert.fail('Never reuse an lifecycle trial stage'); }
...
for (const [name, expected] of Object.entries(reviewed.files)) {
    ...
    assert.equal(hash(bytes[name]), expected, 'Lifecycle review drift: ' + name);
}
...
assert.equal(composition.profile, 'lifecycle');
...
assert.deepEqual(composition.counts, {'companion-first': 33, 'companion-last': 33, 'appload-last': 33});
