...
const trial = '20260923T103000Z-1', stage = 'build/probe-' + trial;
const sourcePath = path.resolve('ops/stage-lifecycle.mjs');
...
test('uncleared lifecycle stage rejects every request before evidence reads or writes', () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'companion-lifecycle-uncleared-'));
    try {
        for (const id of [trial, '20260922T233500Z-1', '../escape']) {
            ...
            assert.match(r.stderr, /Lifecycle disposable trial requires a fresh exact-capsule review/);
            assert.deepEqual(fs.readdirSync(root), []);
        }
    } finally { fs.rmSync(root, {recursive: true, force: true}); }
});
function fixture() {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'companion-lifecycle-staging-'));
    ...
    fs.cpSync('build/lifecycle-native', root + '/build/lifecycle-native', {recursive: true});
    ...
    const compositionPath = root + '/build/lifecycle-native/composition.json';
...
test('exact lifecycle fixture stages only eleven private payloads with a reproducible manifest', () => {
    const f = fixture();
    try {
...
for (const name of names) test('lifecycle stage refuses exact payload drift: ' + name, () => {
    const f = fixture();
    try {
        fs.appendFileSync(name === 'base.sha256' ? f.localBase : f.root + '/build/lifecycle-native/' + name, 'drift');
        assert.notEqual(f.run().status, 0); assert(!fs.existsSync(f.root + '/' + stage));
    } finally { f.close(); }
});
test('lifecycle stage refuses changed qualification/composition fields', () => {
    for (const [key, value] of [['status', 'target-pass'], ['profile', 'load'], ['firmware', '3.29.0.999'],
        ...
        ['counts', {'companion-first': 33, 'companion-last': 33}]]) {
        const f = fixture();
        try {
...
test('lifecycle stage requires actual matching bootstrap target evidence with unchanged service PIDs', () => {
    const changes = [
...
test('lifecycle stage refuses wrong trial, partial/dangling stages and nonregular evidence', () => {
    for (const mode of ['id', 'partial', 'dangling', 'host-link', 'ruler-link', 'base-link',
        ...
        try {
            ...
            if (mode === 'partial') { fs.mkdirSync(dir); fs.writeFileSync(dir + '/keep', 'untouched'); }
                ...
                const file = {'host-link': f.root + '/build/lifecycle-native/NativeHost.qml',
                    'ruler-link': f.root + '/build/lifecycle-native/SizeRuler.qml', 'base-link': f.localBase,
                ...
                const file = f.root + '/build/lifecycle-native/NativeHost.qml'; fs.unlinkSync(file); fs.mkdirSync(file);
            }
